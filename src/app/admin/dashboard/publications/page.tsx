...
const empty: Omit<Publication, 'id'> = {
  title: '', authors: '', conference: '', date: '', link: null, display_order: 0,
}
...
export default function AdminPublications() {
  ...
  async function fetchData() {
    const { data } = await supabase.from('publications').select('*').order('display_order')
    setItems(data ?? [])
    setLoading(false)
  }
  ...
  async function handleSave() {
    if (!editing) return
    if (editing.id) {
      await supabase.from('publications').update(editing).eq('id', editing.id)
    } else {
      await supabase.from('publications').insert(editing)
    }
    setEditing(null)
    fetchData()
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0d0d0d', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2rem', background: 'rgba(13,13,13,0.9)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <span style={{ color: '#fff', fontWeight: 700, fontSize: '0.9rem' }}>Publications</span>
        </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '6rem 1.5rem 4rem', position: 'relative', zIndex: 1 }}>
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          <h1 style={{ color: '#fff', fontWeight: 800, fontSize: '2rem', margin: 0 }}>Publications</h1>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {items.map((item) => (
            <div key={item.id} style={{ padding: '1.25rem 1.5rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', borderLeft: '3px solid #a855f7' }}>
              <div style={{ flex: 1 }}>
                <div style={{ color: '#fff', fontWeight: 700, fontSize: '0.9rem', lineHeight: 1.5 }}>{item.title}</div>
                <div style={{ color: '#a855f7', fontSize: '0.8rem', marginTop: '0.2rem' }}>{item.conference} · {item.date}</div>
              </div>
        ...
        {editing && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
            <div style={{ background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '560px', maxHeight: '85vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '1.1rem', margin: 0 }}>{editing.id ? 'Edit Publication' : 'Add Publication'}</h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <textarea value={editing.title ?? ''} onChange={e => setEditing({ ...editing, title: e.target.value })} rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
                </div>
                {[
                  { key: 'authors', label: 'Authors' },
                  { key: 'conference', label: 'Conference' },
                  { key: 'date', label: 'Date (e.g. November 2024)' },
                  { key: 'link', label: 'GitHub / Link (optional)' },
                ].map(({ key, label }) => (
                  <div key={key}>
